...
export function CalculatorExperience({
  ...
  const activeVariant = useMemo(
    ...
    [definition.variants, variantField, values],
  );
  const resultState = useMemo(
    () => definition.run(values),
          ...
          {tool.faqItems.map((faq) => (
            <Card key={faq.question} className="p-0">
              <details className="group px-6 py-5">
                <summary className="flex cursor-pointer select-none items-center justify-between gap-4 text-base font-semibold text-[var(--color-text)] [list-style:none] [&::-webkit-details-marker]:hidden">
                  {faq.question}
                  <svg
                    aria-hidden="true"
                    className="h-4 w-4 shrink-0 text-[var(--color-text-muted)] transition-transform duration-150 group-open:rotate-180"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                    <path
